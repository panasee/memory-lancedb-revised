# MEMORY.md 中 LanceDB Revised Plugin 相关规则摘录

> 以下内容全部摘自 `/home/ubuntu/clawd/MEMORY.md`，仅保留与 LanceDB / memory-lancedb-revised 插件直接相关的条目。
...
## Rule 10 — 编辑前确认目标代码库
When working on memory plugins, confirm you are editing the intended package (e.g., `memory-lancedb-revised` vs built-in `memory-lancedb`) before making changes; use `memory_recall` + filesystem search to avoid patching the wrong repo.
